...
const deriveKey = (secret, roomName) => {
	...
	return crypto.subtle.importKey('raw', secretBuffer, 'PBKDF2', false, [ 'deriveKey' ]).then((keyMaterial) =>
...
const decrypt = (data, key) => {
	...
	return crypto.subtle
		...
		.then((decryptedValue) => decoding.readAny(decoding.createDecoder(new Uint8Array(decryptedValue))));
};
   ...
   */
     ...
     */
		...
		this.peer.on('connect', () => {
			...
			room.provider.emit('connected', [ remotePeerId ]);
			this.connected = true;
			// send sync step 1
			const provider = room.provider;
		...
		this.peer.on('close', () => {
			...
			room.provider.emit('closed', [ remotePeerId ]);
			checkIsSynced(room);
			this.peer.destroy();
